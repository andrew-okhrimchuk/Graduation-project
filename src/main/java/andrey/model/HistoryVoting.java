...
@NamedQueries({
        ...
        @NamedQuery(name = HistoryVoting.GET_VOTING_BY_DATE_Between, query = "SELECT u FROM HistoryVoting u WHERE u.isSecondVotin=false AND u.dateTime>=:start AND u.dateTime<=:end ORDER BY u.dateTime DESC"),
        @NamedQuery(name = HistoryVoting.GET_VOTING_BY_DATE_BY_USER_ID, query = "SELECT u FROM HistoryVoting u WHERE u.dateTime>=:dateTimeStart AND u.dateTime<=:dateTimeEnd AND u.id=:userId"),
